...
- If draft status prevents required CI from running, mark the implemented PR ready once local checks pass, while keeping merge gated on CI and review. Read failures, fix their cause, commit, and push before collecting fresh evidence.
...
5. After a fix, rerun the applicable local gate, push, wait for fresh CI, and run `code-review` on the updated PR diff. Give reviewers prior findings and their dispositions for revalidation. Re-review all newly changed code; do not merely declare the old review valid.
...
Complete all technical work before asking for merge approval. Once the following facts hold, present the concrete PR URL, source and target branches, head SHA, a concise change summary, CI/review results, and any material consequences. Ask the user to confirm merging this PR and wait for their answer:
...
- The worktree has no uncommitted task fixes, and the remote head is the exact head whose CI and review passed.
- Expected CI checks for the current candidate passed; both review axes completed and have no unresolved blocking findings.
...
After confirmation, re-read head/base and provider gate status immediately before merging. If the head changed, restart CI/review for the new head and obtain new confirmation for the changed result. If the base advanced, refresh the comparison and integration evidence; update the branch when required and recheck/review any resulting changes. A changed integration result or material consequence requires renewed confirmation. Do not resolve unknown or conflicting gate state optimistically.

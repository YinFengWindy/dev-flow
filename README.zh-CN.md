...
确认点之间，agent 自动处理测试方案、开发、CI 排障和 review 修复。PR 的 head 提交变化后，需要重新检查、判断是否需要 review，并重新获得合并确认。
...
需要支持本地 skills 的 Codex、Git 和已认证的 issue/PR connector 或 CLI。需要独立 review 时才要求子 agent 能力；免 review 的改动不依赖该能力。目标项目本身的开发依赖需要正常安装。
